...
class PL_Platform_Updater {
  ...
  function check_info_popup( $false, $action, $arg ){
    ...
    if( ! is_object( $arg ) || ! is_array( $data ) || ! isset( $arg->slug ) || ! isset( $data[$arg->slug] ) )
      return false;
  ...
  function notice_updates_needed() {
    ...
    $link = sprintf( '<a href="%s">%s &rarr;</a>',
      PL_Platform()->oauth->connect_account_link(),
      __( 'Connect This Site', 'pl-platform' )
      );
  ...
  function injectUpdatePlugins( $updates ) {
    ...
    foreach( $pl_plugins as $path => $plugin ) {
      ...
      // NOTE ignore this plugins path, let WordPress handle it!
      // TODO this must be enabled to allow updates via wp.org
      // if( 'pl-platform' == $slug ) {
      //   continue;
      // }
