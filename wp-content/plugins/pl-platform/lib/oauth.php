...
class PL_Platform_OAuth{
  ...
  function __construct( PL_Platform $platform ){
    ...
    if( isset( $_GET['code'] ) && isset( $_GET['page'] ) && 'pl-platform-account' == $_GET['page'] ) {
   ...
   */
  function primary_authorization( $code ){

    $args = array(
      'token'      => $code,
      'redirect_uri' => esc_url( $this->platform->url() )
    );
    ...
    wp_safe_redirect( $this->platform->url() );
  }
