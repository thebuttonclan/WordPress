...
function get_section_data( $s, $config = array() ){
    ...
    if( isset( $results[ $uid ] ) )
        $settings = $results[ $uid ];

    /** If user config'd then thats next best */
    else{
...
function pl_dynamic_media( $config = array()  ){
    ...
    <div class="media-wrap <?php echo $classes;?>" data-bind="visible: <?php echo $config['key'];?>() || <?php echo $htmlkey;?>()">
        ...
        <img src="<?php echo $config['src'];?>" alt="<?php echo $config['alt'];?>" data-bind="plimg: <?php echo $config['key'];?>" />
...
function pl_replace_query_terms( $taxonomy ){
  ...
  if( $taxonomy == 'postformat' )
    $query_tax = 'post_format';

  elseif( $taxonomy == 'posttag' )
    $query_tax = 'post_tag';
...
function pl_fallback_image( $size = 'aspect-thumb', $shortcode = false ){
  ...
  elseif( $size == 'basic-thumb' || $size == 'thumbnail' ){
    ...
    $url = pl_framework_url('images').'/default-image.png';
...
function pl_page_title(){
  ...
  elseif( is_home() )
    $title = __('Blog', 'pl-platform');

  elseif( is_search() )
    $title = __('Search', 'pl-platform');

  elseif( is_category() )
    $title = __('Category', 'pl-platform');

  elseif( is_tag() )
    $title = __('Tag', 'pl-platform');

  elseif( is_author() )
    $title = __('Author', 'pl-platform');

  elseif( is_archive() )
    $title = __('Archive', 'pl-platform');

  elseif( is_404() )
    return __('404 Error!', 'pl-platform');
...
function pl_default_widget($id, $default = false ){
    ...
    if( isset($wp_registered_sidebars[ $id ]) && isset($wp_registered_sidebars[ $id ]['name']) )
        $name = sprintf( '"%s"', $wp_registered_sidebars[ $id ]['name']);
    else{
    ...
    <div id="comments" class="wp-comments">
        ...
        <?php
            ...
            if ( post_password_required() ){
                printf('<p class="nopassword">%s</p></div></div>', __( 'This post is password protected. Enter the password to view any comments.', 'pl-platform' ) );
                return;
            }
